...
import com.koolearn.framework.search.declare.MySearchOption;
import com.koolearn.framework.search.declare.MySearchOption.DataFilter;
...
public class SearchServiceImp implements SearchService, InitializingBean, DisposableBean {
    ...
    private QueryBuilder createSingleFieldQueryBuilder(String field, Object[] values, MySearchOption mySearchOption) {
        try {
            if (mySearchOption.getSearchType() == com.koolearn.framework.search.declare.MySearchOption.SearchType.range) {
                /*区间搜索*/
            ...
            for (Object valueItem : values) {
                ...
                if (mySearchOption.getSearchType() == com.koolearn.framework.search.declare.MySearchOption.SearchType.term) {
                    queryBuilder = QueryBuilders.termQuery(field, formatValue).boost(mySearchOption.getBoost());
                }
                else if (mySearchOption.getSearchType() == com.koolearn.framework.search.declare.MySearchOption.SearchType.querystring) {
...
//                    .put("client.transport.ignore_cluster_name", "true")
    ...
    private Map<String, String> _group(String indexName, QueryBuilder queryBuilder, String[] groupFields) {
        try {
            ...
            for (org.elasticsearch.search.facet.terms.TermsFacet.Entry entry : termsFacet.entries()) {
                result.put(entry.getTerm(), entry.count() + "");
            }
            return result;
        }
        catch (Exception e) {
